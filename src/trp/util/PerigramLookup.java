...
public class PerigramLookup implements ReaderConstants {
	...
	public PerigramLookup(String... textFileNames) {

		this(textFileNames, Readers.guessFileNames(textFileNames));
	}

	public PerigramLookup(String[] textFileContents, String[][] trigramFileData) {
		
		doHashing(textFileContents, trigramFileData);
	}
	
	public PerigramLookup(String theText, String[] trigramRows) {
		
		doHashing(theText, trigramRows);
	}

	public PerigramLookup(String[] textFileNames, String[] rawPerigramFiles) {

		long ts = System.currentTimeMillis();
		String[] textContents = Readers.loadFiles(textFileNames);
		String[][] trigramData = Readers.loadTrigramsFiles(rawPerigramFiles);
		Readers.info("Loaded data files in " + RiTa.elapsed(ts));
		doHashing(textContents, trigramData);
	}

	private void doHashing(String text, String[] trigramRows) {

		long ts = System.currentTimeMillis();
		computeBigrams(text);
		if (PROFILE) Readers.info("Hashed bigrams in " + RiTa.elapsed(ts));
		ts = System.currentTimeMillis();
    computePerigrams(trigramRows);
    if (PROFILE) Readers.info("Hashed perigrams in " + RiTa.elapsed(ts));
		addStopCCWords();
	}
	
	private void doHashing(String[] textFileContents, String[][] trigramFileData) {

		long ts = System.currentTimeMillis();
		bigramMap = hashBigrams(textFileContents);
		if (PROFILE) Readers.info("Hashed bigrams in " + RiTa.elapsed(ts));
		ts = System.currentTimeMillis();
    perigramMap = hashPerigrams(trigramFileData);
    if (PROFILE) Readers.info("Hashed perigrams in " + RiTa.elapsed(ts));
		addStopCCWords();
	}

	private void addStopCCWords() {
	...
	public boolean isPerigram(int countThreshold, RiText... rts) {
		...
		if (!checkPeriNulls(rts)) return false;
	...
	private boolean checkPeriNulls(RiText... rts) {
		...
		if (rts[0] == null || rts[1] == null || rts[2] == null) {
			...
			// Readers.warn("Null word in perigram array: " + more); // doesnt cause
			// problem?
			return false;
		}
		else
	...
	// a boolean: whether or not there is a non-zero count
	{
		...
		int count = 0;
		verifyPerigramArray(rts);
		if (!checkPeriNulls(rts)) return count; // 0 if any of the cells are null
		String[] words = prepareWordsForLookups(rts);
	...
	public int getCount(String... words) {
		...
		int count = 0;
		if (perigramMap.containsKey(periKey)) count = perigramMap.get(periKey);
	...
	private void verifyPerigramArray(RiText... rts) {
		...
		if (rts.length != 3) Readers.error("Bad array size=" + rts.length + ", expecting 3");
	}
	...
	public Map<String, Integer> hashBigrams(String[] textFileContent) {

		for (int j = 0; j < textFileContent.length; j++) {
			computeBigrams(textFileContent[j]);
		}
		// System.out.println(bigramMap.size()+" unique bigrams");//"+bigramMap);
	...
	private void computeBigrams(String txt) {
		...
		if (bigramMap == null) bigramMap = new HashMap<String, Integer>();
	...
	public Map<String, Integer> hashBigramsOld(PApplet p, String[] textNames) {
		...
		for (int j = 0; j < textNames.length; j++) {
			...
			for (int i = 0; i < rows.length; i++) {
				...
				if (firstWord == null) firstWord = words[0];
				if (lineLast != null) bigramMap.put(lineLast + " " + words[0], 0);
				lineLast = null;
				...
				if (lineLast == null) lineLast = bigram[1];
			}
	...
	public Map<String, Integer> hashPerigrams(String[][] trigramFilesContent) {
		
		perigramMap = new HashMap<String, Integer>();
		
		for (int j = 0; j < trigramFilesContent.length; j++) {
			String[] rows = trigramFilesContent[j];
			computePerigrams(rows);
		}
		return perigramMap;
	}

	private void computePerigrams(String[] rows) {
	...
	public Map<String, Integer> hashPerigrams(PApplet p, String[] rawTrigramsFile) {
		...
		for (int j = 0; j < rawTrigramsFile.length; j++) {
			trigramFilesContent[j] = p.loadStrings(rawTrigramsFile[j]);
		}
		return hashPerigrams(trigramFilesContent);
	}
