...
public class RiTextGrid {
	...
	public static float DEFAULT_GRID_ALPHA = 255;
	public static float[] DEFAULT_GRID_FILL = { 0, 0, 0, DEFAULT_GRID_ALPHA };
	...
	protected float gridAlpha = DEFAULT_GRID_ALPHA;
	...
	public RiTextGrid(PApplet pApplet, String fileName, int startX, int startY, int NOTUSED) {
		this(pApplet, RiText.createLines(pApplet, RiTa.loadString(/* pApplet, */fileName), startX, startY, pApplet.width - startX * 2, pApplet.height - startY * 2));
	}
	...
	public RiTextGrid(PApplet pApplet, PageLayout layout) {
		...
		if (layout != null) {
			...
			if (lines == null || lines.length < 1) Readers.error("Cannot create a grid with no text!");
	...
	public static int computeMaxWordLength() {
		...
		for (Iterator it1 = instances.iterator(); it1.hasNext();) {
			...
			for (Iterator it = grid.iterator(); it.hasNext();) {
				...
				int rtl = rt.length();
				if (rtl > maxWordLength) maxWordLength = rtl;
			}
		}
	...
	public void buildGrid(RiText[] lines) {
		...
		for (Iterator it = iterator(); it.hasNext();)
			((RiText) it.next()).fill(DEFAULT_GRID_FILL);
		...
		if (memoizeNeighbors) neighborCache = new HashMap();
		instances.add(this);
	...
	protected void buildTemplateCell(RiText[][] rts) {
		...
		template.fill(DEFAULT_GRID_FILL);
		template.position(Float.MAX_VALUE, Float.MAX_VALUE); // this makes it
																													// invisible
		// template.alpha(0); // don't do this: messes with fades back to
		// DEFAULT_GRID_FILL
	}
	...
	public String toString() {
		...
		if (pm != null) {
			if (this == pm.getNext())
				...
				name = "verso";
			else if (this == pm.getRecto()) name = "recto";
		}
		if (1 == 1)
			return "grid(" + name + ") #" + id;
		else
	...
	public void visible(boolean b) {
		...
		gridAlpha = b ? DEFAULT_GRID_ALPHA : 0;

		// Readers.info("RiTextGrid.visible("+b+")");
		for (Iterator it = iterator(); it.hasNext();) {
			((RiText) it.next()).alpha(gridAlpha);
		}

		if (title != null) title.alpha(gridAlpha);
		if (footer != null) footer.alpha(gridAlpha);
	}
	...
	public RiText[] wordsMatching(String word) {
		...
		for (Iterator it = iterator(); it.hasNext();) {
			RiText rt = (RiText) it.next();
			if (word.equals(rt.text())) l.add(rt);
		}
	...
	public static RiTextGrid getGridFor(RiText rt) {
		...
		for (Iterator it = instances.iterator(); it.hasNext();) {
			...
			if (g.contains(rt)) return g;
		}

		Readers.warn("Illegal state in RiTextGrid.getGridFor(), no grid for RiText: " + rt);
		// throw new RuntimeException("Illegal state in RiTextGrid.getGridFor(), no
		// grid for RiText: " + rt);
	...
	private Point coords(RiText rt) {
		...
		for (int i = 0; i < cells.length; i++) {
			for (int j = 0; j < cells[i].length; j++) {
				if (cells[i][j] == rt) return new Point(j, i);
			}
		}
		return null;
	}
	...
	private RiText nextCell(int x, int y) {
		...
		if (next == null) {
			...
			Readers.error("RiTextGrid.nextCell(" + x + "," + y + ")->" + cellAt(x, y) + " returned " + "null for " + this + " [verso=" + verso + " recto=" + recto + " newX=" + newX + " newY=" + newY + "]");
		}
	...
	private RiText previousCell(int x, int y) {
		...
		Readers.verify(cellAt(x, y) != null, "previousCell(" + x + "," + y + ") -> no cell at x=" + x + ", y=" + y + " for grid:\n" + g);

		// try prev word on the line
		int newY = y, newX = x - 1;

		// are we at beginning of line?
		if (newX < 0) newY--; // if so, first reduce Y
		...
		if (newY < 0) {
			...
			if (pg == null) Readers.error("Null previous grid in previousCell(" + x + "," + y + ")" + g);

			// WHY DOES THIS HAPPEN???
			if (g == pg) ;// Readers.warn("previousCell() -> Previous grid is
										// identical to grid...");
		...
		if (rt == null) {
			boolean recto = (g == PageManager.getInstance().getRecto());
			boolean verso = (g == PageManager.getInstance().getVerso());
			Readers.error("RiTextGrid.previousCell(" + x + "," + y + ")->" + cellAt(x, y) + " returned " + "null for " + this + " [verso=" + verso + " recto=" + recto + " newX=" + newX + " newY=" + newY + "]");
		}
	...
	/**
	 * Returns the 9-cell neighborhood for the specified cell ([4]), as follows:
	 * <br>
	 * 
	 * <pre>
	 * [0] [1] [2]
	...
	public RiText[] neighborhood(final RiText center) {
		...
		else // last line, so wrap
		{
			...
			for (int i = firstLine.length - 1; i >= 0; i--) {
				if (gnext.intersectsOnX(center, firstLine[i], 3)) under = firstLine[i];
			}
			if (under == null) {
				// TODO: check in new code: this was the warning previously
				// Readers.warn("Illegal state, " + "nothing under: " + center + " (on
				// next grid)");

				// This should only happen when 'center' is a last word of a long line
				// and so there really isn't anything 'under' it, so set under to the
				// first word
				// of the first line on the next grid and give a let warning for the
				// time being
				// DEBUG Readers.warn("Exceptional state, " + "nothing under: \"" +
				// center.text() + "\" (on next grid)");
				// DEBUG Readers.info("'under' set to first word of the first line on
				// the next grid.");
				under = firstLine[0];
		...
		if (memoizeNeighbors) neighborCache.put(center, rts);
	...
	public List bestBelows(RiText rt, float slop) {
		...
		if (p1.y < numLines() - 1) {
			...
			while (it.hasNext()) {
				...
				p2 = coordsFor(test);
				if (p1.y == p2.y - 1 && intersectsOnX(rt, test, slop)) l.add(test);
			}
		}
		return l;
	}
	...
	public RiText[] lineAt(int _y) {
		...
		if (cells == null || _y < 0 || _y > cells.length - 1) return null;
		return cells[_y];
	}
	...
	public void resetLine(int lineIdx) {
		...
		if (cells == null || lineIdx < 0 || lineIdx > cells.length - 1) return;
	...
	public void setColor(float[] color, boolean updateTemplate) {

		setColor(color[0], color[1], color[2], color[3], updateTemplate);
	}
	...
	public void shiftText(float xOffset) {
		...
		if (title != null) title.x += xOffset;
		if (footer != null) footer.x += xOffset;
	}
	...
	public String[][] toStringArray(boolean updateWithCurrentGridWord) {
		...
		if (origWords == null || updateWithCurrentGridWord) origWords = createStringArray();
		return origWords;
	}
	...
	public String getIdTag() {
		...
		String nm = "";
		if (name != null && name.length() > 0) nm = "%20(" + name + ")";
		return "#" + gridId + nm;
	}
	...
	public void setTitle(String s, int x, int y, PFont pf) {
		...
		if (title == null) title = new RiText(_pApplet, x, y);
		if (pf != null) title.font(pf);
	...
	public void setFooter(String s, int x, int y) {
		...
		if (footer == null) footer = new RiText(_pApplet);
	...
	public static void defaultColor(int r, int g, int b, int a) {
		...
		DEFAULT_GRID_FILL = new float[] { r, g, b, a };
		defaultAlpha(a);
	}

	public static void defaultAlpha(int a) {
	...
	private RiText getClosestCell(RiText rt) {
		...
		if (rt == null) throw new RuntimeException("Bad Cell: " + rt);
		RiText best = null;
	...
	public static int yDistance(RiText rt1, RiText rt2) {
		...
		if (grid1 == grid2) return (y2 < y1) ? Integer.MAX_VALUE : (y2 - y1);

		if (grid1.next != grid2) return Integer.MAX_VALUE;
