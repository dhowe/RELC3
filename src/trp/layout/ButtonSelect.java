...
public class ButtonSelect implements ReaderConstants {
	...
	public static float[] STROKE = BLACK, TEXT_FILL = WHITE, FILL, HOVERFILL = { 255, 255, 255, 64f }, LABELFILL = OATMEAL;
	public static int PADDING = 6, STROKE_WEIGHT = 0, TEXT_SIZE;

	// these can be set directly (after creation) to manipulate appearance of one
	// button
	...
	public boolean hidden, disabled;
	...
	private void setProps() {
		...
		if (height == 0) {
			height = (int) (p.textAscent() + p.textDescent() + ((padding - 2) * 2)); // KLUDGE:
																																								// -
																																								// 2
		}
	}
	...
	public static ButtonSelect click(int mx, int my) {
		...
		return click(mx, my, false);
	}

	// assumes no overlap between buttons
	public static ButtonSelect click(int mx, int my, boolean isShiftDown) { 

		for (Iterator it = instances.iterator(); it.hasNext();) {
			ButtonSelect button = (ButtonSelect) it.next();
			if (!button.disabled && button.contains(mx, my)) {
				return (isShiftDown) ? button.backup() : button.advance();
			}
		}
		return null;
	}
