...
{ 
  ...
  public static PageManager create
    (PApplet pApplet, int leftMargin, int topMargin,  int rightMargin, int bottomMargin)
  {
    if (instance == null) 
    {
...
/*      if (spotlightMode != SPOTLIGHT_NONE) {
        ...
        instance = new PageManager(pApplet, leftMargin, topMargin,
            rightMargin, bottomMargin, pApplet.width/2, pApplet.height);
     // }
  ...
  protected PageManager(PApplet pApplet, int leftMargin, int topMargin, 
      int rightMargin, int bottomMargin, int pageWidth, int pageHeight)
  {
    this(pApplet, new Rect(leftMargin, topMargin, 
      pageWidth-(leftMargin+rightMargin), pageHeight-(topMargin+bottomMargin)),
...
*/
  ...
  private void addPages(String txt)
  {
    ...
    while (hasNext(text)) {// create the grid layouts
...
/*  private void renderPageToBuffer(PApplet p, GLGraphicsOffScreen page, RiTextGrid rtg, boolean transparent)
  ...
  public void onGridChange
    (MachineReader changed, RiTextGrid changedFrom, RiTextGrid changedTo)
  {
