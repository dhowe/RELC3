...
public class PageManager implements BehaviorListener, ReaderConstants, PConstants {
	...
	public static int spotlightMode = SPOTLIGHT_NONE; // ALL, NONE or
																										// SPOTLIGHT_FOCUS
	...
	public static PageManager create(PApplet pApplet, int leftMargin, int topMargin, int rightMargin, int bottomMargin) {
		...
		if (instance == null) {
			...
			/*
			 * if (spotlightMode != SPOTLIGHT_NONE) { instance = new
			 * ShadingPageManager(pApplet, leftMargin, topMargin, rightMargin,
			 * bottomMargin, pApplet.width/2, pApplet.height); } else {
			 */
			instance = new PageManager(pApplet, leftMargin, topMargin, rightMargin, bottomMargin, pApplet.width / 2, pApplet.height);
			// }
	...
	private PageManager(PApplet p, PageLayout rpl) {
		if (p == null) Readers.error("Null PApplet");
		this._pApplet = p;
		this.pageLayout = rpl;
		is3D = false; // ??
		useGLGraphics = false;// is3D && p.ginstanceof
													// codeanticode.glgraphics.GLGraphics;
	...
	public void draw(PGraphics g) {
		...
		if (rotateY < nextVisibleAt) next.draw(g);
	...
	/**
	 * Returns true if the word is on the verso page as a new page is flipping
	 * over it
	 */
	...
	private RiTextGrid buildPage(StringBuilder text) {
		...
		if (font == null) font = RiText.defaultFont(this._pApplet);

		if (leading == -1) leading = font.getSize() * RiText.defaults.leadingFactor;
		...
		if (idx > -1) // deal with page break
		{
			...
			// System.out.println("FORCED PAGE BREAK!");//
		...
		/*
		 * yuk -- need uniform way of dealing w' headers/footers, they are currently
		 * in both PageLayout and Grid
		 */
		if (showPageNumbers)
	...
	public void showAll() {
		...
		if (left != null) {
			left.setColor(left.DEFAULT_GRID_FILL, true);
			left.visible(true);
		}
		if (right != null) {
			right.setColor(left.DEFAULT_GRID_FILL, true);
			right.visible(true);
			if (next != null) {
				next.setColor(left.DEFAULT_GRID_FILL, true);
				next.visible(true);
			}
		}
		showTitles();
	...
	/**
	 * Returns a list of all the RiTexts in the set of pages managed by this
	 * PageManager.
	 */
	public RiText[] doLayout() {
		...
		if (pages.size() < 3) disablePageFlips = true;
		...
		if (pages.size() > 2) next = (RiTextGrid) pages.get(2);
	...
	public RiText[] getAllWords() {
		...
		if (allWords == null) throw new RuntimeException("Must call doLayout() before getAllWords()...");
		return allWords;
	}
	...
	public void clear() {
		...
		if (pages != null) {
			for (Iterator it = pages.iterator(); it.hasNext();) {
				RiTextGrid grid = (RiTextGrid) it.next();
				grid.dispose();
			}
			pages = new ArrayList();
		}
	...
	private void addPages(String txt) {
		...
		while (hasNext(text)) {
			...
			if (rtg != null) pages.add(rtg);
		}
	}

	public void behaviorCompleted(RiTextBehavior behavior) {

		if (flipping && behavior == lerp) onNextPageComplete();
	}
	...
	/*
	 * private void renderPageToBuffer(PApplet p, GLGraphicsOffScreen page,
	 * RiTextGrid rtg, boolean transparent) { // draw to the buffer (3D) if (page
	 * != null) { page.beginDraw(); page.background(255); rtg.draw(page);
	 * page.endDraw();
	 * 
	 * int sx=200, sy=200; //spotlight center
	 * 
	 * GLTexture tex = page.getTexture(); tex.loadPixels(); // Spotlight Calcs ***
	 * for (int i = 0; i < tex.pixels.length; i++) { tex.pixels[i] = 0xffff0000; }
	 * tex.loadTexture();
	 * 
	 * p.beginShape(); p.texture(tex); p.vertex(0, 0, 0, 0); p.vertex(page.width,
	 * 0, page.width, 0); p.vertex(page.width, page.height, page.width,
	 * page.height); p.vertex(0, page.height, 0, page.height);
	...
	private void onNextPageComplete() {
		...
		if (focusedReader != null) {
			// DEBUG -remove Readers.info("unpausing with: " +
			// focusedReader.getCurrentCell().text());
	...
	public void onGridChange(MachineReader currentReader, RiTextGrid grid, RiTextGrid gridForNextWord) {
		...
		if (grid == right && gridForNextWord == next) // DEBUG had: || changedFrom
																									// == next
		{
			// DEBUG -remove: Readers.warn("page-turn caused by: " +
			// currentReader.getCurrentCell().text());
		...
		// + changedFrom + " to="
		// + changedTo + " (this shouldn't happen on a text with more than 2
		// pages!)");
		// }
	}
	...
	public String addTextsFromFile(String... fileNames) {

		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < fileNames.length; i++)
			sb.append(addTextFromFile(fileNames[i]));
		return sb.toString();
	}

	public String addTextFromFile(String fileName) {
	...
	public float screenX(RiTextGrid grid, RiText cell) {
		...
		float x = cell.x; // yuk
		if (grid == right || grid == next) x += cell.getPApplet().width / 2f;
		return x;
	}
	...
	private void applyStyle(StyledText st) {
		...
		if (st.leading > -1) setLeading(st.leading);
		if (st.paragraphLeading > -1) setParagraphSpacing(st.paragraphLeading);
		if (st.indents > -1) setParagraphSpacing(st.indents);
	...
	public void onUpdateFocusedReader(MachineReader reader) {
		...
		// ALSO: in future js iterations? move all non-visible readers to the verso
		// page
	...
	private void addSpotlight(PImage p, MachineReader... readers) {
		...
		if (spotlightRadius > 0 && readers != null) {
			...
			for (int i = 0; i < readers.length; i++) {
				...
				if (grid != right && grid != left && !(grid == next && flipping)) continue;
				...
				float lerpTime = (millis - spotlightTimers[i]) / (readers[i].getSpeed() * 1000); // 0
																																													// -1
				...
				if (grid == right || grid == next) newX[i] += (p.width / 2f - gutterSubtraction);
			}
		...
		for (int x = 0; x < p.width; x++) {
			...
			for (int y = 0; y < p.height; y++) {
				...
				for (int i = 0; i < sx.length; i++) {
					float dist = (float) Math.sqrt((((sx[i] - xpos)) * ((sx[i] - xpos))) + (sy[i] - ypos) * 2 * (sy[i] - ypos));// -
																																																											// centerSpotlightSize;
					if (dist < minDist) minDist = dist;
				}

				// 'adjustBrightness' ranges from 0-1 with closest/brightest approaching
				// 1

				// float adjustBrightness = 1 / (1f + minDist);
				float adjustBrightness = Math.min(maxSpotlightBrightness, Math.max(minSpotlightBrightness, (radius - minDist) / radius));
	...
	public RiText[] getHeaders() {
		...
		for (Iterator it = pages.iterator(); it.hasNext();) {
			RiTextGrid page = (RiTextGrid) it.next();
			if (page.title != null) l.add(page.title);
		}
	...
	public void isUsingServerFocus(boolean sendServerFocus) {
		...
		if (sendServerFocus && MachineReader.SERVER_HOST == null) Readers.warn("Server=false, FocusThread=true...");
